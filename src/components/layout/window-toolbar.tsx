      ...
      {sidebarOpen && updatesCount > 0 && (
        <Tooltip label={t("updates.tooltip")}>
          <button className="toolbar-btn toolbar-btn-update" onClick={onToggleUpdates}>
            <img src={updateIcon} alt="" style={{ width: "var(--icon-md)", height: "var(--icon-md)" }} />
          </button>
        </Tooltip>
      )}
      {!sidebarOpen && (
