...
fn chat_mode_injects_chat_prompt() {
    ...
    prepare_messages(&mut msgs, wd, false, None, true, None, &[], TEST_MODEL_LARGE, "chat", "");
    let sys = &msgs[0];
...
fn chat_mode_skips_agent_md() {
    ...
    prepare_messages(&mut msgs, wd, false, None, true, agent_md, &[], TEST_MODEL_LARGE, "chat", "");
    let sys = &msgs[0];
...
fn chat_mode_skips_skills() {
    ...
    prepare_messages(&mut msgs, wd, false, None, true, None, &skills, TEST_MODEL_SMALL, "chat", "");
    let sys = &msgs[0];
...
fn chat_mode_small_model_gets_compact() {
    let mut msgs = vec![make_user_msg("hello")];
    let wd = std::path::Path::new("/tmp/project");
    prepare_messages(&mut msgs, wd, false, None, true, None, &[], TEST_MODEL_SMALL, "chat", "");
    let sys = &msgs[0];
    assert!(sys.content.contains("conversational assistant"));
