...
pub fn is_data_dir_write(tool_name: &str, args: &Value, working_dir: &Path) -> bool {
    ...
    path_is_inside_data_dir(&resolve_path(path_str, working_dir))
}
...
fn is_bash_data_dir_write(args: &Value, working_dir: &Path) -> bool {
    ...
    path_is_inside_data_dir(working_dir) || command_mentions_data_dir(command)
}

fn command_mentions_data_dir(command: &str) -> bool {
    let data_dir = crate::services::paths::data_dir();
    let data_dir = normalize_path(&data_dir);
    let data_path = data_dir.to_string_lossy();
    command.contains(data_path.as_ref()) || command.contains(".local/share/cl-go-dash")
}

fn path_is_inside_data_dir(path: &Path) -> bool {
