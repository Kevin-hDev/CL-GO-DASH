...
pub async fn run(
    ...
    let result = run_inner(
        app, child_session_id.clone(), model, provider, prompt,
        subagent_type, &parent_emitter, cancel, project_id,
    ).await;
...
async fn run_inner(
    ...
    let messages = vec![
        ...
        ChatMessage { role: "user".to_string(), content: prompt.clone(), ..Default::default() },
    ];
    ...
    let result = crate::commands::agent_chat_task::run_stream_task(
        emitter, child_session_id.clone(), model, messages, tools,
        false, provider,
        Some(working_dir.to_string_lossy().to_string()),
        None, None, Some("subagent".to_string()), cancel.clone(),
    ).await;
    ...
    match result {
        ...
        Err(e) if was_cancelled || e == "Annulé" => {
            Ok((false, "cancelled".to_string(), "Sous-agent annulé.".to_string()))
        }
        Err(e) => Err(e),
    }
}
...
fn extract_summary_from_messages(msgs: &[ChatMessage]) -> String {
    ...
    let tool_results: Vec<&str> = msgs.iter().rev().take(6)
...
async fn resolve_working_dir(
    project_id: Option<&str>, child_session_id: &str, is_explorer: bool,
) -> std::path::PathBuf {
