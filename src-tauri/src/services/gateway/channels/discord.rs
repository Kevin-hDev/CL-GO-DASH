...
impl DiscordAdapter {
    pub fn new() -> Self {
        Self {
            ...
            state: Arc::new(RwLock::new(DiscordState { bot_token: None, bot_user_id: String::new() })),
        }
    }
    ...
    fn to_inbound(
        msg: &DiscordMessage, key: &ChannelKey, require_mention: bool, bot_user_id: &str,
    ) -> Option<InboundMessage> {
        if msg.is_from_bot() || msg.content.is_empty() { return None; }
        let is_group = !msg.is_dm();
        if is_group && require_mention && !msg.mentions_user(bot_user_id) { return None; }
        Some(InboundMessage {
            channel_key: key.clone(), user_id: msg.author.id.clone(),
            content: msg.content.clone(), message_id: msg.id.clone(),
            chat_id: msg.channel_id.clone(), is_group,
...
impl ChannelAdapter for DiscordAdapter {
    ...
    fn capabilities(&self) -> ChannelCapabilities { ChannelCapabilities::discord() }
        ...
        Ok(tokio::spawn(async move {
            loop {
                ...
                let token = {
                    ...
                    match &s.bot_token { Some(t) => t.clone(), None => break }
                };
                ...
                while let Some(Ok(WsMessage::Text(txt))) = stream.next().await {
                    if cancel.is_cancelled() { break; }
                    let Ok(payload) = serde_json::from_str::<GatewayPayload>(&txt) else { continue };
                    if let Some(s) = payload.s { seq = Some(s); }
                    match payload.op {
                        10 => {
                            if let Some(d) = &payload.d {
                                if let Ok(hello) = serde_json::from_value::<GatewayHello>(d.clone()) {
                                    let interval = Duration::from_millis(hello.heartbeat_interval);
                                    tokio::spawn(heartbeat_loop(shared_sink.clone(), cancel.clone(), interval, seq));
                                }
                            }
                            let json = serde_json::to_string(&build_identify(&token)).unwrap_or_default();
                            let _ = shared_sink.lock().await.send(WsMessage::Text(json.into())).await;
                        }
                        ...
                        0 if payload.t.as_deref() == Some("MESSAGE_CREATE") => {
                            if let Some(d) = payload.d {
                                if let Ok(msg) = serde_json::from_value::<DiscordMessage>(d) {
                                    ...
                                    if let Some(inbound) = Self::to_inbound(&msg, &key, require_mention, &bot_id) {
    ...
    async fn send(&self, msg: OutboundMessage) -> GatewayResult<DeliveryReceipt> {
        let token = {
            ...
            s.bot_token.clone().ok_or_else(|| GatewayError::auth("pas de token"))?
        };
        let url = format!("https://discord.com/api/v10/channels/{}/messages", msg.chat_id);
        let body = SendMessage {
            content: msg.content, allowed_mentions: AllowedMentions { parse: vec![] },
            message_reference: msg.reply_to.map(|id| MessageReference { message_id: id }),
        };
        let resp: SentMessage = self.client.post(&url)
            .header("Authorization", format!("Bot {}", token.as_str()))
            .json(&body).send().await.map_err(|e| GatewayError::network(format!("send: {e}")))?
            .json().await.map_err(|e| GatewayError::network(format!("parse: {e}")))?;
        Ok(DeliveryReceipt { message_id: resp.id })
    }

    async fn health(&self) -> bool { self.state.read().await.bot_token.is_some() }
}
